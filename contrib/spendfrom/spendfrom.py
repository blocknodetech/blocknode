...
#
# Assumes it will talk to a blocknoded or blocknode-Qt running
# on localhost.
#
# Depends on jsonrpc
#
...
def determine_db_dir():
    """Return the default location of the blocknode data directory"""
    if platform.system() == "Darwin":
        return os.path.expanduser("~/Library/Application Support/Blocknode/")
    elif platform.system() == "Windows":
        return os.path.join(os.environ['APPDATA'], "Blocknode")
    return os.path.expanduser("~/.blocknode")

def read_bitcoin_config(dbdir):
    """Read the blocknode.conf file from dbdir, returns dictionary of settings"""
    ...
    config_parser.readfp(FakeSecHead(open(os.path.join(dbdir, "blocknode.conf"))))
    return dict(config_parser.items("all"))

def connect_JSON(config):
    """Connect to a blocknode JSON-RPC server"""
    testnet = config.get('testnet', '0')
    testnet = (int(testnet) > 0)  # 0/1 in config file, convert to True/False
    if not 'rpcport' in config:
        config['rpcport'] = 37004 if testnet else 37002
...
def list_available(blocknoded):
    ...
    for output in unspent:
        ...
        # This code only deals with ordinary pay-to-blocknode-address
...
def main():
    ...
    parser.add_option("--datadir", dest="datadir", default=determine_db_dir(),
                      help="location of blocknode.conf file with RPC username/password (default: %default)")
